...
## Facts
These are variables that contain information pertinent to the current host (`inventory_hostname`). They are only available if gathered first. See `vars_and_facts` for more information.

    ansible_facts
        Contains any facts gathered or cached for the `inventory_hostname`
        Facts are normally gathered by the `setup ` module automatically in a play, but any module can return facts.
    ...
    ansible_local
        ...
        See the `setup ` module and `local_facts` for more details.
...
See `general_precedence_rules` for how connection variables interact with `configuration settings`, `command-line options`, and `playbook keywords`.
