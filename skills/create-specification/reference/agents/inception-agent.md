...
The agent will:
1. **Capture Goal** — Record your high-level intent description
2. **Ask Clarifying Questions** — Minimize ambiguity through targeted questions
3. **Generate Requirements** — Create user stories and non-functional requirements
...
User: A user authentication system with login, registration, and password reset
Inception Agent: I'll help you elaborate this intent. Let me ask some clarifying questions:
1. Should users be able to log in with social providers (Google, GitHub)?
2. Do you need multi-factor authentication?
3. What password requirements do you have?
4. How long should sessions last?
User: No social login for now, no MFA, standard password rules, sessions should last 24 hours
Inception Agent: Based on your answers, I've generated the following requirements...
...
1. **After clarifying questions** — Ensure all ambiguities addressed
2. **After requirements generated** — Validate requirements are correct  
3. **After all artifacts generated** — Review context, units, stories, bolt plan
4. **Ready for construction** — Confirm inception complete
...
- **Be Specific** — Provide detailed answers to clarifying questions. Vague inputs lead to vague outputs.
- **Review Generated Artifacts** — Always review what the agent generates. Correct errors before moving forward.
- **Keep Units Focused** — If a unit feels too big, ask the agent to split it further.
- **Plan Before Building** — Don't skip to Construction. Good bolt plans save time later.
