...
1. **Tech Stack** — Languages, frameworks, databases, infrastructure choices
2. **Coding Standards** — Formatting, linting, naming conventions, testing strategy
3. **System Architecture** — Architecture style, API design, state management patterns
4. **UX Guide (Optional)** — Design system, styling, accessibility guidelines
5. **API Conventions (Optional)** — API style, versioning, response formats
...
- **New User** → Explain AI-DLC → Project Init
- **Returning User** → Analyze Context → Route to appropriate agent (Inception, Construction, or Operations)
...
- **Starting a New Project** — Run `project-init` to establish standards
- **Unsure Where to Go** — Use `route-request` to find the right agent
- **Need Orientation** — Run `analyze-context` to see project state
- **Learning AI-DLC** — Use `explain-flow` for methodology guidance
...
- **Start Here** — For new projects, always start with the Master Agent and `project-init`.
- **Check Context Regularly** — Use `analyze-context` to understand where you are in the flow.
- **Ask for Routing** — When unsure, ask the Master Agent. It knows the methodology.
