#!/usr/bin/env python3
"""
Comprehensive evaluation harness for the OpenCode skill-creator meta-skill.

This script implements the full evaluation workflow described in the agent-skills documentation:
1. Runs baseline (no skill) and with-skill versions in parallel
2. Captures timing data (total_tokens, duration_ms)
3. Prepares outputs for human review and assertion drafting
4. Generates benchmark.json and report.md
"""
...
WORKSPACE_DIR = BASE_DIR / "skill-creator-workspace"
...
def main():
    ...
    print("Starting skill-creator evaluation...")
    ...
    if isinstance(evals_data, list):
        ...
        skill_name = evals_data.get("skill_name", "skill-creator")
    ...
    for i, test_case in enumerate(test_cases):
        ...
        print(f"  Baseline: {baseline_result['duration_ms']}ms, {baseline_result['total_tokens']} tokens")
        print(f"  Skill: {skill_result['duration_ms']}ms, {skill_result['total_tokens']} tokens")
...
def generate_benchmark(results: list, output_dir: pathlib.Path):
    ...
    baseline_times = [r["baseline"]["duration_ms"] for r in results if r["baseline"]["success"]]
    skill_times = [r["with_skill"]["duration_ms"] for r in results if r["with_skill"]["success"]]
    baseline_tokens = [r["baseline"]["total_tokens"] for r in results if r["baseline"]["success"]]
    skill_tokens = [r["with_skill"]["total_tokens"] for r in results if r["with_skill"]["success"]]
    ...
    benchmark = {
        "run_summary": {
            "baseline": {
                ...
                "time_seconds": {
                    "mean": sum(baseline_times) / len(baseline_times) / 1000.0 if baseline_times else 0,
                    "stddev": 0
                },
                "tokens": {
                    "mean": sum(baseline_tokens) / len(baseline_tokens) if baseline_tokens else 0,
                    "stddev": 0
                }
            },
            "with_skill": {
                ...
                "time_seconds": {
                    "mean": sum(skill_times) / len(skill_times) / 1000.0 if skill_times else 0,
                    "stddev": 0
                },
                "tokens": {
                    "mean": sum(skill_tokens) / len(skill_tokens) if skill_tokens else 0,
                    "stddev": 0
                }
            },
            "delta": {
                "pass_rate": (skill_passes / total_tests if total_tests > 0 else 0) - (baseline_passes / total_tests if total_tests > 0 else 0),
                "time_seconds": (sum(skill_times) / len(skill_times) / 1000.0 if skill_times else 0) - (sum(baseline_times) / len(baseline_times) / 1000.0 if baseline_times else 0),
                "tokens": (sum(skill_tokens) / len(skill_tokens) if skill_tokens else 0) - (sum(baseline_tokens) / len(baseline_tokens) if baseline_tokens else 0)
            }
        }
    }
...
def generate_report(results: list, output_dir: pathlib.Path):
    ...
    lines.append("")
    lines.append("| Test ID | Prompt | Baseline Time (s) | Skill Time (s) | Baseline Success | Skill Success |")
    lines.append("|---------|--------|-------------------|----------------|------------------|---------------|")
    ...
    for result in results:
        ...
        lines.append(f"| {result['test_id']} | {prompt_display} | {baseline_time:.2f} | {skill_time:.2f} | {baseline_success} | {skill_success} |")
    ...
    for result in results:
        ...
        lines.append(f"- Status: {'Success' if result['baseline']['success'] else 'Failed'}")
        ...
        lines.append(f"- Status: {'Success' if result['with_skill']['success'] else 'Failed'}")
