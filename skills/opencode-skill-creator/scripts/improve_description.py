...
by calling `opencode run` as a subprocess (uses the session's OpenCode auth,
...
def _call_opencode(prompt: str, model: str | None, timeout: int = 300) -> str:
    ...
    Uses a temporary file to pass the prompt since it embeds the full SKILL.md
    body and can exceed comfortable command line length.
    """
    ...
    try:
        cmd = ["opencode", "run", f"--file={prompt_file}"]
        if model:
