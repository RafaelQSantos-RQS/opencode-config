...
author: "Steve Sewell"
site: "Builder.io"
published: 2025-09-09T07:00:00.000Z
source: "https://www.builder.io/blog/agents-md"
domain: "builder.io"
language: "en"
description: "Stop re-prompting. Put the rules in AGENTS.md: do and don’ts, file-level tests, and real examples so agents ship code that matches your project."
word_count: 2477
---

If you have ever used an AI agent to generate code and thought, “this is so smart, and so dumb at the same time,” this is for you. A small `AGENTS.md` file can make a big difference to get your agents to behave the way you want.

Below are my best tips for writing an effective `AGENTS.md`. We will start with a simple example that has none, then layer in each section with what it does and why it helps. At the end we will combine everything into a final `AGENTS.md`.

## What is an AGENTS.md

[AGENTS.md](http://agents.md/) is a small markdown file at the root of your repo that tells AI tools how your project works. Similar to tool-specific formats like `.cursorrules`, `.builderrules`, etc - which can clutter up your project with one-per-tool files, `AGENTS.md` is a growing standard for putting rules that should apply to any agents in one place.

The file is basically a README for AI agents, and its supported by [many of them](https://agents.md/). Instead of repeating yourself in every prompt, you write the defaults once and let the agent start from there.

## Results without an AGENTS.md

Here is an example I tried with no `AGENTS.md`. I took a Figma design and pasted it into the [Builder.io agent](https://www.builder.io/fusion?_host=www.builder.io) and said: add this as an app-wide tab.
...
Editing [Builder.io](https://www.builder.io/) with [Builder.io](https://www.builder.io/) (Builderception?) is all fun and games until AI does dumb stuff.
...
![](https://www.youtube.com/watch?v=KEK_WcSTiuE)

### Dos and Don’ts

The main thing to do to create a good [`AGENTS.md`](https://agents.md/) is just do some trial and error with your code. Run some prompts, look at what you like and don’t like, and start compiling a list of dos and don’ts accordingly.
...
Note: you can use MCP servers to expose docs not in your code as well, and you can write guidance on how and when to use MCP servers in your [`AGENTS.md`](http://agents.md/) too.
...
### You can nest AGENTS.md files

Large repos benefit from hierarchical level rules. Add an `AGENTS.md` in each critical sub directory so guidance matches its exact stack and version. The agent reads the closest file to the work it is doing.
...
If your design system lives in a separate package, the agent will guess how to use it. [Index your system](https://www.builder.io/c/docs/component-indexing) so the agent learns component APIs, tokens, and examples. Then reference that output here.
...
## Results with an AGENTS.md

Now that we’ve added a lot more guidance for the AI, lets look at how the [Builder.io agent](https://www.builder.io/fusion?_host=www.builder.io) does with the same prompt we tried earlier.
...
The best part of it all is with tool like [Builder.io](http://builder.io/) an `AGENTS.md` is particularly helpful, because unlike purely engineering-focused tools like Cursor and Claude Code, Builder is commonly used by designers and product managers as well, so these rules become especially valuable to make sure when those non-developers are working with code, they are given great defaults and guidelines by default.

## But what about CLAUDE.md, etc

While many tools support `AGENTS.md`, not all do (yet). The solution though is easy - agents can look up files so one file can point to another file. For tools that don’t yet support the standard, you can have them simply point to your `AGENTS.md`, like:
...
If you want to be fancy, you can also just symlink `CLAUDE.md` (etc) to `AGENTS.md` too.
...
Keep your `AGENTS.md` small and scoped. Lead with concrete examples and file paths. Give file scoped commands for type check, format, lint, and tests. Iterate and add a rule the second time you see the same mistake.

If you want to try all these out in an agent you get instant visual feedback on to quickly experiment with rules on your existing code - try out [Fusion](https://fusion.builder.io/) and send me your feedback.
...
[Try Builder](https://builder.io/signup) [Get a demo](/m/demo)

<video autoplay muted controls loop preload="metadata" class="builder-video css-cz4v3a"></video>
